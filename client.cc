...
int QueueLength = 5;
std::string filepath = "/mnt/c/Users/lsqro/AppData/Roaming/.minecraft/saves";

// Processes time request
void processSyncRequest( int socket );
...
int main( int argc, char ** argv )
{
  ...
  error = connect( serverSocket, &serverIPAddress, sizeof(serverIPAddress));
  if ( error ) {
    perror("listen");
    exit( -1 );
  }

  std::string * filename = new std::string("hello.txt\r\n");
  write(serverSocket, filename->c_str(), strlen(filename->c_str()));
  
}

void processSyncRequest( int fd )
{
